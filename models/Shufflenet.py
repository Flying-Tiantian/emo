    ...
    def __stage(self, x, stage=2, repeat=3):
        if 2 <= stage <= 4:
            stage_layer = shufflenet_unit('stage' + str(stage) + '_0', x=x, w=None,
                                          ...
                                          num_filters=
                                          self.output_channels[str(self._num_groups)][
                                              stage - 2],
                                          stride=(2, 2),
                                          fusion='concat', l2_strength=self._weight_decay,
                                          bias=self._bias,
                                          batchnorm_enabled=self._batchnorm_enabled,
            ...
            for i in range(1, repeat + 1):
                stage_layer = shufflenet_unit('stage' + str(stage) + '_' + str(i),
                                              ...
                                              num_filters=self.output_channels[
                                                  str(self._num_groups)][stage - 2],
                                              stride=(1, 1),
                                              fusion='add',
                                              l2_strength=self._weight_decay,
                                              bias=self._bias,
                                              batchnorm_enabled=self._batchnorm_enabled,
                                              is_training=self.is_training)
            return stage_layer
    ...
    def __call__(self, x, trainable=True, is_training=True, reuse=False):
        ...
        with tf.variable_scope(self._name, reuse=reuse):
            ...
            conv1 = conv2d('conv1', x=x_padded, w=None, num_filters=self.output_channels['conv1'], kernel_size=(3, 3),
                        stride=(2, 2), l2_strength=self._weight_decay, bias=self._bias,
                        batchnorm_enabled=self._batchnorm_enabled, is_training=self.is_training,
            ...
            logits_unflattened = conv2d('fc', global_pool, w=None, num_filters=self._num_classes,
                                        kernel_size=(1, 1),
                                        l2_strength=self._weight_decay,
                                        bias=self._bias,
                                        is_training=self.is_training)
