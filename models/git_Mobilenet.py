...
def mobilenet_v1_arg_scope(
  ...
  with slim.arg_scope([slim.conv2d, slim.separable_conv2d, slim.batch_norm],
                      trainable=trainable):
...
class Mobilenet_model:
  ...
  def __call__(self, x, trainable=True, is_training=True, reuse=False):
    with slim.arg_scope(mobilenet_v1_arg_scope(trainable=trainable)):
      outputs, _ = mobilenet_v1(x, num_classes=self._num_classes, is_training=is_training, reuse=reuse, scope=self._name)
...
class Mobilenet_160_model(Mobilenet_model):
  def __init__(self, num_classes=10):
    self._num_classes = num_classes
    self._name = 'MobilenetV1_160'
    self._default_input_size = 160

class Mobilenet_64_model(Mobilenet_model):
  def __init__(self, num_classes=10):
    self._num_classes = num_classes
    self._name = 'MobilenetV1_64'
    self._default_input_size = 64

if __name__ == '__main__':
    
  model = Mobilenet_model(num_classes=7)
  inputs = tf.placeholder(tf.float32, shape=[1, 64, 64, 1])
  logits = model(inputs)
  pass